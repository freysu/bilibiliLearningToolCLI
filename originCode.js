...
async function run() {
  ...
  async function chooseRunMode() {
    try {
      tipToast("如果不联网，请先访问https://api.bilibili.com/x/web-interface/view?bvid=你要查的BV号，将网页内容保存成.json格式文件到当前目前下，然后本工具将会读取它\n例如：https://api.bilibili.com/x/web-interface/view?bvid=BV1GL4y1v79M")
            ...
            const { inputJsonPath } = await askQuestions([
              {
                ...
                type: 'input',
                message:
                  '请输入本地的json文件路径(如果是放在同文件夹下可直接输"XXX.json")：',
                filter: function (val) {
                  if (val.indexOf('.json') !== -1) {
                    return path.join(__dirname, val.trim())
                  } else {
                    return path.join(__dirname, (val += '.json'))
                  }
                }
              }
            ])
            successToast(`正在读取JSON文件!其文件绝对路径：${inputJsonPath}`)
            try {
              data = fs.readFileSync(inputJsonPath, 'utf8')
...
//  * @param {number} pos 要计算的位置
//  * @returns 计算结果或者错误提示字符串
//  */
